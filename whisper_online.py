...
class ASRBase:
    # join transcribe words with this character
    # (" " for whisper_timestamped, "" for faster-whisper because it emits the spaces when needed)
    sep = " "
...
class WhisperTimestampedASR(ASRBase):
    ...
    def transcribe(self, audio, init_prompt=""):
        result = whisper_timestamped.transcribe_timestamped(self.model, audio, language=self.original_language,
...
class HypothesisBuffer:
    ...
    def insert(self, new, offset):
        # compare self.committed_in_buffer and new.
        # It inserts only the words in new that extend the committed_in_buffer,
        ...
        if len(self.new) >= 1:
            ...
            if abs(a - self.last_committed_time) < 1:
                if self.committed_in_buffer:
                    ...
                    # that are identical in commited and new. If they are, they're dropped.
...
class OnlineASRProcessor:
    ...
    def prompt(self):
        """Returns a tuple: (prompt, context), where "prompt" is a 200-character suffix of committed text
        that is inside the scrolled away part of audio buffer.
    ...
    def process_iter(self):
        ...
        # this was an attempt to trim silence/non-linguistic noise detected
        # by the fact that Whisper doesn't transcribe anything for 3-times in a row.
    ...
    def chunk_completed_segment(self, res):
        ...
        if len(ends) > 1:
            ...
            if e <= t:
                ...
                print(f"--- last segment not within committed area", file=sys.stderr)
        else:
...
if __name__ == "__main__":
    ...
    # asr = WhisperASR(lan=language, model_size=size)
    ...
    asr = asr_cls(model_size=size, lan=language, cache_dir=args.model_cache_dir, model_dir=args.model_dir)
